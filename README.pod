...
Because the online bookmarks manager pearltrees (www.pearltrees.com) offers less & less satisfation to its user, I decided to create a script to make of my existing bookmarks. However, the only possible export file is a RDF file. So I needed at first to retrieve the data from the file.
